...
public:
	...
	bool removeData(const std::string& dataName, const std::string& filename = "",
			bool pruneEmptyFiles = false) override;

	// searches the map for all the data sets of the given name and returns them as a list
	std::map<std::string, std::list<std::shared_ptr<ghost::SaveData>>> getData(
	    const std::string& dataName) const override;
